...
static int get_bootloader_message_mtd(struct bootloader_message *out,
    ...
    int offset = write_size * MISC_COMMAND_PAGE;

    /* Guarantee data buffer size to be larger than a bootloader_message size.
     * If write_size is below 512, data buffer size can be smaller than
     * a bootloader_message size. */
    while ((size - offset) < (ssize_t)sizeof(*out))
        size += write_size;
...
static int set_bootloader_message_mtd(const struct bootloader_message *in,
     ...
     * If write_size is below 512, data buffer size can be smaller than
     * a bootloader_message size. */
    while ((size - offset) < (ssize_t)sizeof(*in))
        size += write_size;
    ...
    memcpy(&data[offset], in, sizeof(*in));
