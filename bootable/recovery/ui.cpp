...
RecoveryUI::RecoveryUI() :
    ...
    if (property_get("ro.hardware", device_name, NULL)) {
        if (!strncmp(device_name, "loki", 4) ||
            !strncmp(device_name, "foster", 6)) {
            one_button_recovery = true;
        }
    }
}
...
void RecoveryUI::process_key(int key_code, int updown) {
    ...
    if (register_key) {
        if (one_button_recovery) {
            if (time_key_press < SCROLL_TIME)
                key_code = KEY_SCROLL;
            else if (time_key_press <= TOGGLE_TIME)
                key_code = KEY_TOGGLE;
            else if (time_key_press > TOGGLE_TIME)
                key_code = KEY_INVOKE;
        }
