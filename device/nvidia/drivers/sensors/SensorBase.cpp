...
int SensorBase::enable(int32_t handle, int enabled, int channel)
{
    ALOGI_IF(FUNC_ENTRY,
             "%s handle=%d enabled=%d channel=%d\n",
             __func__, handle, enabled, channel);
    return 0;
}

int SensorBase::setDelay(int32_t handle, int64_t ns, int channel)
{
    ALOGI_IF(FUNC_ENTRY,
...
{
    ALOGI_IF(FUNC_ENTRY, "%s handle=%d\n", __func__, handle);
    return -EINVAL;
}

int SensorBase::getSensorList(struct sensor_t *list, int index, int limit)
{
    ALOGI_IF(FUNC_ENTRY,
...
int SensorBase::getFd(int32_t handle)
{
    ALOGI_IF(FUNC_ENTRY, "%s handle=%d\n", __func__, handle);
    if (!data_name) {
        return dev_fd;
...
int SensorBase::initFd(struct pollfd *pollFd, int32_t handle)
{
    ALOGI_IF(FUNC_ENTRY,
...
int SensorBase::readRaw(int32_t handle, int *data, int channel)
{
    ALOGI_IF(FUNC_ENTRY,
             "%s handle=%d data=%p channel=%d\n",
             __func__, handle, data, channel);
    return -1;
}
...
int SensorBase::openInput(const char *inputName)
{
    ...
    *filename++ = '/';
    while((de = readdir(dir))) {
        ...
        ALOGV_IF(EXTRA_VERBOSE, "path open %s", devname);
        ...
        if (fd >= 0) {
            ...
            ALOGV_IF(EXTRA_VERBOSE, "name read %s", name);
...
int SensorBase::batch(int handle, int flags,
                      int64_t period_ns, int64_t timeout_ns)
{
    int ret;

    if (timeout_ns || (flags & SENSORS_BATCH_WAKE_UPON_FIFO_FULL))
        return -EINVAL;

    if ((!timeout_ns) && (!(flags & SENSORS_BATCH_DRY_RUN)))
