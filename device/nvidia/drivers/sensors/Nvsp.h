 ...
 * #define SENSORS_DEVICE_API_VERSION   ex: SENSORS_DEVICE_API_VERSION_1_3
 */
...
private:
    ...
    int getDevNum(const char *dev_name, enum NVS_DRIVER_TYPE driverType) {
            ...
            if ((d = readdir(dp)) != NULL) {
                ...
                if (!ret) {
                    ...
                    if (fd >= 0) {
                        ...
                        if (!ret) {
                            /* test if device already found before */
                            for (i = 0; i < DRIVER_COUNT_MAX; i++) {
                                if (mSensor[i].devNum == devNum &&
                                                      mSensor[i].nvsp_i >= 0) {
                                    nvsp_i = mSensor[i].nvsp_i;
                                    if (!nvspDriverList[nvsp_i].devType)
                                        /* multiple device OK if devType==0 */
                                        continue;

                                    if (nvspDriverList[nvsp_i].
                                                    driverType == driverType) {
                                        ALOGI_IF(SensorBase::EXTRA_VERBOSE,
                                                 "%s %s already found "
                                                 "devNum=%d driverType=%d "
                                                 "nvspDriverList[%d]\n",
                                                 __func__, dev_name, devNum,
                                                driverType, mSensor[i].nvsp_i);
                                        break;
                                    }
                                }
                            }
                            if (i < DRIVER_COUNT_MAX)
                                continue;
    ...
    void dbgSensorList(unsigned int i) {
        ...
        ALOGI("%s sensor_t[%u].stringType=%s\n",
              __func__, i, nvspSensorList[i].stringType);
        ALOGI("%s sensor_t[%u].requiredPermission=%s\n",
              __func__, i, nvspSensorList[i].requiredPermission);
        ALOGI("%s sensor_t[%u].maxDelay=%d\n",
              __func__, i, (int)nvspSensorList[i].maxDelay);
        ALOGI("%s sensor_t[%u].flags=%u\n",
              __func__, i, (unsigned int)nvspSensorList[i].flags);
...
sensors_poll_context_t::sensors_poll_context_t()
{
    ...
    if (!ARRAY_SIZE(nvspDriverList)) {
        ALOGI("%s empty nvspDriverList. Exiting\n", __func__);
        return;
    }
    ...
    for (handle = 0; handle < DRIVER_COUNT_MAX; handle++) {
        ...
        mSensor[handle].fd = -1;
        mSensor[handle].devNum = -1;
        mSensor[handle].nvsp_i = -1;
        mSensor[handle].fusionClient = false;
        mSensor[handle].enabled = false;
 ...
 */
    ...
    for (unsigned nvsp_i = 0; nvsp_i < ARRAY_SIZE(nvspDriverList); nvsp_i++) {
        ...
        if (nvspDriverList[nvsp_i].devName != NULL) {
            devNum = getDevNum(nvspDriverList[nvsp_i].devName,
                               nvspDriverList[nvsp_i].driverType);
            if (devNum < 0)
                /* device not found */
                continue;
        } else {
        ...
        if (nvspDriverList[nvsp_i].newDriver == NULL) {
            mSensor[handle].driver = new Nvs(devNum,
                                             nvspDriverList[nvsp_i].devName,
                                             nvspDriverList[nvsp_i].devType,
                                             nvspDriverList[nvsp_i].driverType,
                                             NULL);
        } else {
        ...
        if (nvspDriverList[nvsp_i].staticHandle) {
            ...
            if (nvspDriverList[nvsp_i].noList) {
                i = mSensor[handle].driver->getSensorList(nvspSensorList,
                                                          noListHandle - 1, 1);
                if (i > 0) {
                    handleEnd = handle + 1;
                    noListHandle--;
        ...
        /* fill in mSensor[i] */
        for (i = handle; i < handleEnd; i++) {
            mSensor[i].devNum = devNum;
            mSensor[i].nvsp_i = nvsp_i;
    ...
    if (mSensor[0].driver == NULL) {
        /* if no fusion driver then disable fusion */
        for (handle = 0; handle < DRIVER_COUNT_MAX; handle++)
            mSensor[handle].fusionClient = false;
    }

    /* make sure all sensors have a driver */
    for (handle = 1; handle <= numSensors; handle++) {
        if (mSensor[handle].driver)
            continue;

// FIXME: just remove the sensor if no driver and compact the sensor list.
// Problem is that we don't want to change the nvspSensorList that drivers
// have a pointer to their sensor(s) in.
// We can either rerun getSensorList for every driver, or come up with a
// pointer-to-a-pointer mechanism.  Probably the former is best.
        numSensors = 0;
    ...
    if (noListHandle < SENSOR_COUNT_MAX) {
        for (handle = noListHandle + 1; handle < DRIVER_COUNT_MAX; handle++) {
            if (mSensor[handle].driver)
                continue;

// FIXME: just remove the sensor if no driver
// This one we can probably just remove the driver since the list isn't
// given to anyone.  In fact, we may be able to remove this check altogether.
            numSensors = 0;
            ALOGE("%s ERR: no sensor or fusion driver! Aborting\n", __func__);
            return;
        }
