...
static void set_affinity_daemon_enable(struct powerhal_info *pInfo, int value)
{
    if (value == 1)
...
static void app_profile_set(struct powerhal_info *pInfo, app_profile_knob *data)
{
    ...
    for (i = 0; i < APP_PROFILE_COUNT; i++)
    {
        switch (i) {
            ...
            case APP_PROFILE_AFFINITY_DAEMON_ENABLE:
                set_affinity_daemon_enable(pInfo, data[i]);
                break;
            default:
                break;
        }
